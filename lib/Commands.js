...
class Commands {
    ...
    async list(msg){
        var message = '*Voici la liste des différents modules* : \n';
        await this.bot.sendMessage(msg.chat.id, message, {parseMode: 'Markdown'})
        message = ""
        var messages = []
        await Object.keys(modules).forEach( async (el, idx, ary) => {
            if(modules[el].category != 'Non développement d\'applications 2014'){
                message += '• `' + el + '` - ' + modules[el].name + ' [📄](' + modules[el].link + ')' + '\n';
            }
            if ((idx + 1) % 50 === 0 & idx !== 0){
                console.log("envoi");
        ...
        for await (let element of messages) {
            await this.bot.sendMessage(msg.chat.id, element, {parseMode: 'Markdown'})
        }
    }
    all(msg){this.list(msg)}
    listall(msg){this.list(msg)}
    commandes(msg){this.list(msg)}
